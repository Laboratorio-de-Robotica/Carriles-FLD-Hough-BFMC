...
while(True):
    ...
    zenithals = det.zenithalSegmentsFactory(segments, hui.H @ euclidean)
    ...
    # zenithal view
    zenithalIm = cv.warpPerspective(im, hui.H, (hui.zenithalSquareSide, hui.zenithalSquareSide))
    cv.imshow('zenithal', zenithalIm)

    zenithalIm2 = cv.warpPerspective(im, hui.H2, hui.zenithalSize)
    # autoshrink
    while(zenithalIm2.shape[0] > 700):
        print('zenithalIm2.shape', zenithalIm2.shape)
        #cv.resize(zenithalIm2, (zenithalIm2.shape[1]//2, zenithalIm2.shape[0]//2))
