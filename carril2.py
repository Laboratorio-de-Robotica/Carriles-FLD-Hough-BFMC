...
while(True):
    ...
    if(key != -1):
        ...
        match chr(key):
            case ' ':
                play = not play
            case 'p':
                print(hui.roiPoly, type(hui.roiPoly), type(hui.roiPoly[0]), type(hui.roiPoly[0][0]))
            case 's':
                save()
            case 'l':
                load()
    ...
    mainSegmentsIndices = bins.getIndicesFromBin(houghSpacePerspectiveWeigthed.maxLoc)
